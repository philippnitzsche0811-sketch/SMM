...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    GRANT SELECT, INSERT, UPDATE, DELETE, REFERENCES ON ALL TABLES IN SCHEMA public TO socialhub_app;
    GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO socialhub_app;

    ALTER DEFAULT PRIVILEGES IN SCHEMA public
        GRANT SELECT, INSERT, UPDATE, DELETE, REFERENCES ON TABLES TO socialhub_app;
    ALTER DEFAULT PRIVILEGES IN SCHEMA public
